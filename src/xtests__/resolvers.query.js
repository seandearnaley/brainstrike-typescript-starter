...
describe("[Query.launches]", () => {
  ...
  it("calls lookup from launch api", async () => {
    ...
    const res = await Query.launches(null, {}, mockContext);
    expect(res).toEqual({
      cursor: "foo",
      hasMore: false,
  ...
  it("respects pageSize arg", async () => {
    ...
    // check the resolver response
    const res = await Query.launches(null, { pageSize: 1 }, mockContext);
    expect(res).toEqual({
      cursor: "bar",
      hasMore: true,
  ...
  it("respects cursor arg", async () => {
    ...
    // check the resolver response
    const res = await Query.launches(null, { after: "b" }, mockContext);
  ...
  it("respects both pageSize and cursor", async () => {
    ...
    // check the resolver response
    const res = await Query.launches(
      null,
      { after: "c", pageSize: 1 },
      mockContext
    );
...
describe("[Query.launch]", () => {
  ...
  it("calls lookup from launch api", async () => {
    ...
    // check the resolver response
    const res = await Query.launch(null, { id: 999 }, mockContext);
...
describe("[Query.me]", () => {
  ...
  it("returns null if no user in context", async () => {
    expect(await Query.me(null, null, mockContext)).toBeFalsy();
  });
  ...
  it("returns user from userAPI", async () => {
    ...
    const res = await Query.me(null, null, mockContext);
    expect(res).toEqual({
      id: 999
    });
  });
